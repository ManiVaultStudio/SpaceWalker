...
void SpaceWalkerPlugin::mousePositionChanged(Vector2f mousePos)
{
    ...
    int selectedPoint = findClosestPointToMouse(_scatterPlotWidget->getPointRenderer(), _dataStore.getProjectionView(), bounds, widgetDimensions, mousePos, _mask.empty() ? full : _mask);
